...
//    when(backOfficeRoleRepository.findFirstByNameAndActiveOrderByCreatedDate("SampleRole", true))
...
//
//    BackOfficeRole role = roleService.mapRoleDTOToEntity(roleDTO);
//    when(backOfficeRoleRepository.findFirstByNameAndActiveOrderByCreatedDate(roleDTO.getName(),true)).thenReturn(Optional.ofNullable(role));
//
//    roleService.deleteRole(roleDTO.getName());
//    when(backOfficeRoleRepository.findFirstByNameAndActiveOrderByCreatedDate(roleDTO.getName(),true)).thenThrow(new ZeusRuntimeException(""));
//
